...
static _i32 SlValidateServerInfo(void)
{
    ...
    if(!strstr((const char *)g_RecvBuf.Buf, "stream:stream"))
    {
        ASSERT_ON_ERROR(SERVER_INFO_INVALID_RESPONSE);
    }

    pServerStr = (_u8 *)strstr((const char *)g_RecvBuf.Buf, "from=");
...
static _i32 SlValidateQueryResult(void)
{
    if(strstr((const char *)g_RecvBuf.Buf, "<success") == 0)
...
static _i32 SlValidateBindFeature(void)
{
    if(strstr((const char *)g_RecvBuf.Buf, "xmpp-bind") == 0)
    {
...
static _i32 SlBindingConfigure(void)
{
    ...
    if(!strstr((const char *)g_RecvBuf.Buf, "result")
            || !strstr((const char *)g_RecvBuf.Buf, "bind"))
    {
        ASSERT_ON_ERROR(BIND_CONFIG_INVALID_RESPONSE);
    }

    pJid = (_u8 *)strstr((const char *)g_RecvBuf.Buf, "<jid>");
    if(!pJid)
        ASSERT_ON_ERROR(BIND_CONFIG_INVALID_RESPONSE);
...
static _i32 SlValidateRoster(void)
{
    ...
    if(!strstr((const char *)g_RecvBuf.Buf, "jabber:iq:roster") ||
       !strstr((const char *)g_RecvBuf.Buf, "roster_1"))
    {
        ASSERT_ON_ERROR(ROSTER_INVALID_RESPONSE);
    }

    pJid = (_u8 *)strstr((const char *)g_RecvBuf.Buf, "item jid=");
    while (pJid != NULL)
    {
        pJid += 10;
        ...
        pMyJid[idx-1] = '\0';
        idx = 0;
        pJid = (_u8 *)strstr((const char *)pJid, "item jid=");
    }
                      ...
                      _u8* pMessage, _u16 Msglen)
{
    ...
    memcpy(ccPtr, MyJid.Buf, strlen((const char *)MyJid.Buf));
    ccPtr += strlen((const char *)MyJid.Buf);
    ...
    *ccPtr= '\0';
    ...
    Status = sl_Send(g_SockID, g_SendBuf.Buf, strlen((const char *)g_SendBuf.Buf), 0);
    if(Status <= 0)
                      ...
                      _u8* pMessage, _u16 Msglen)
{
    ...
    Status = sl_SetSockOpt(g_SockID, SL_SOL_SOCKET, SL_SO_NONBLOCKING,
    ...
    pBasePtr = (_u8 *)strstr((const char *)g_RecvBuf.Buf, "<body>");
    if(!pBasePtr)
    {
    ...
    *pBodyPtr = '\0';
    if (strlen((const char *)pBasePtr)+1 < Msglen)
    {
        memcpy(pMessage, pBasePtr, strlen((const char *)pBasePtr)+1);
    }
    else
    {
        ASSERT_ON_ERROR(XMPP_INVALID_RESPONSE);
    ...
    pBodyPtr = (_u8 *)strstr((const char *)g_RecvBuf.Buf, "from=");
    ...
    *pBodyPtr = '\0';
    ...
    if (strlen((const char *)pFromPtr)+1 < Jidlen)
    {
        memcpy(pRemoteJid, pFromPtr, strlen((const char *)pFromPtr)+1);
        memcpy(RemoteJid.Buf, pFromPtr, strlen((const char *)pFromPtr)+1);
        memcpy(g_RosterJid.Buf, RemoteJid.Buf, strlen((const char *)RemoteJid.Buf)+1);
    }
    else
    {
        ASSERT_ON_ERROR(XMPP_INVALID_RESPONSE);
...
static _i32 SlXmppConnectionSM(void)
{
    ...
    */
    ...
    while(g_XMPPStatus != CONNECTION_ESTABLISHED)
    {
        ...
        if(NeedToReceive == 1 && strlen((const char *)g_RecvBuf.Buf) <= 0)
            continue;
        ...
        switch(g_XMPPStatus)
        {
          ...
          case STARTTLS_RESPONSE_RECV:
            ...
            ccPtr += 65;
            memcpy(ccPtr, g_MyBaseKey.Buf, strlen((const char *)g_MyBaseKey.Buf));
            ccPtr += strlen((const char *)g_MyBaseKey.Buf);
            memcpy(ccPtr, "</auth>", 7);
            ccPtr += 7;
            *ccPtr= '\0';
        ...
        {
            NeedToSend = 0;
            len = strlen((const char *)g_SendBuf.Buf);
